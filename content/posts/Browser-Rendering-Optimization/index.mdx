...
브라우저에서 가장 큰 콘텐츠 요소가 나타날 때의 시간을 뜻한다.

일반적으로 2.5초 이내에 LCP가 렌더링 되는 것이 좋다.

주로 고려해야하는 요소로는 IMG, SVG, VIDEO, 배경 이미지가 있는 URL, 텍스트를 다수 포함한 블록 수준의 요소 등이 있다.

웹페이지는 단계적으로 로드되는 경우가 많으며 그에 따라서 페이지의 최대 요소가 변경 될 수도 있다. 
...
큰 요소가 최대 콘텐츠풀 요소로 유지된다.

LCP 분석을 위해서는 가장 마지막에 확인된 PerformanceEntry만 분석하면 된다.

쉽게 설명하면 가장 이목을 끌 요소가 얼마나 빠르게 나타나는지에 대한 지표다.


LCP가 일어나는 이유와 해결법은 다음과 같다.
...
First Contentful Paint(최초 콘텐츠풀 페인트)는 모든 콘텐츠가 페이지에 렌더링되는지를 나타내는 지표다.
첫 요소들이 확인되는 시간으로 각 요소들의 상호작용과 응답성과 같은 부분은 고려하지 않는다.

FID는 실제 사용자 상호 작용에서 측정되는 지표이기 때문에 실험적으로 확인할 수가 없다.
단, FID를 개선할 수 있는 방법은 있는데 total blocking time(JS에서 각 작업의 50ms가 넘어갈 때의 시간들의 합을 말한다.)을 개선시키는 것이다.

FID를 개선시킬 수 있는 방법은 다음과 같다.
 ...
 즉, JS 연산 시간을 최소한으로 줄이면 FID 지표가 높아진다.

###### CLS

Cumulative Lay Shift(누적 레이아웃 이동)
CLS는 렌더링 된 이후 0.5초 이내에 렌더링 되지 않아 전체적인 콘텐츠가 이동하는 불안정성을 말한다.


CLS가 일어나는 이유는 다음과 같다.
...
미리 공간을 준비하거나 리소스를 미리 다운받아 CLS를 최소화 할 수 있다.


---

 이 부분은 추후 프로젝트를 진행하면서 고려한 것과 고려하지 않은 것의 차이를 비교해볼 필요가 있을것 같다.
