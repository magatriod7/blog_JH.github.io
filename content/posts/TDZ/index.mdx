...
var의 경우 var의 정보가 존재한다고 미리 알려주고 var이 있기는 한데 뭔지는 모르겠어라서
JS에서 undefined 처리를 한 것이지만

let과 const도 호이스팅이 일어나는데 undefined로 default가 정혀져 있지 않습니다. 이는 let과 const의 성질상 초기화하기 전에 읽거나 쓰게 되면 ReferenceError가 생성되는 성질을 가지고 있기 때문입니다.
