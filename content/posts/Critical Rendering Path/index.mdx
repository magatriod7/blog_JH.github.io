...
2 - CSS의 정보를 담은 CSSOM을 생성합니다.

3 - 브라우저 엔진은 위에서 만들어진 DOM과 CSSOM을 결합하여 렌더 트리를 생성합니ㄷ.
...
CRP의 최적화는 렌더링의 시간을 개선시킵니다. CRP를 이해하면 reflow, repaint를 개선하고 버벅거림을 방지할 수 있다.
...
적당히 예시를 든 것인데 다른 상황에서도 적용 가능 할 것입니다.
...
CSS의 원 뜻인 Cascading Style Sheets에서 Cascading답게 CSS 규칙은 토큰을 노드로 변환할 때 아래로 상속된다. 하지만 CSS는 다른 하위 CSS에 의해서 기존 규칙이 덮어져 변경될 수 있습니다. 
CSSOM은 위의 이유로 CSS를 분석할 때 build 되지만 변경될 추후 변경될 수도 있기 때문에 완전 분석 전 까지 렌더링에 사용되지 않는다. 브라우저는 모든 CSS를 처리하고 수신할 때까지 페이지의 렌더링을 방지합니다.
...
paint가 일어나더라도 layout 과정은 일어나지 않을 수도 있습니다.
